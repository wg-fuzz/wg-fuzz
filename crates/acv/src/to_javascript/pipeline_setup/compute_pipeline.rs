    ...
    const {} = {}.createComputePipeline({{
        ...
        compute: {{
            ...
            entryPoint: \"main\"
                    ...
                    compute_pipeline.var_name, device.var_name, compute_pipeline.var_name, pipeline_layout.var_name, shader_module.var_name)
            } else {
        ...
        CreateComputePipelineAsync(device, shader_module, pipeline_layout) => {
            if let Resource::GPUComputePipeline(compute_pipeline) = created_resource {
                format!("\
    const {} = await {}.createComputePipelineAsync({{
        label: \"{}\",
        layout: {},
        compute: {{
            module: {},

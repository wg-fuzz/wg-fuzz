...
pub fn add_manipulate_current_render_pass(available_api_calls: &mut Vec<APICall>, rng: &mut ThreadRng, fuzzy_prob: f64, device: &GPUDevice, command_encoder: &GPUCommandEncoder, all_passes_finished: bool) -> bool {
    ...
    for render_pass in &command_encoder.render_pass_encoders {
        ...
        if let None = &render_pass.pipeline {
            ...
            for bundle in &device.render_bundle_encoders {
                if bundle.finished && rng.gen_bool(0.8) || fuzzy(rng, fuzzy_prob) {
        ...
        } else if !render_pass.drew || fuzzy(rng, fuzzy_prob) {
        ...
        } else if !render_pass.finished && !render_pass.debug_group_active || fuzzy(rng, fuzzy_prob) {
            available_api_calls.extend([EndRenderPass(render_pass.clone())])
        }

        if !render_pass.finished || fuzzy(rng, fuzzy_prob) {
            ...
            if !render_pass.debug_group_active || fuzzy(rng, fuzzy_prob) {
            ...
            if let Some(query_set_num) = render_pass.occlusion_query_set_num {
                if !render_pass.query_active && !device.query_sets[query_set_num].destroyed || fuzzy(rng, fuzzy_prob) {
                    available_api_calls.extend([BeginOcclusionQuery(render_pass.clone(), device.query_sets[query_set_num].clone())])
                } else if render_pass.query_active || fuzzy(rng, fuzzy_prob) {

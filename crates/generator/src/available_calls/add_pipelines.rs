...
pub fn add_create_pipelines(available_api_calls: &mut Vec<APICall>, rng: &mut ThreadRng, device: &GPUDevice) {
    for shader_module in &device.shader_modules {
        if shader_module.compute_or_render.contains("compute") || fuzzy(rng) {
        ...
        } else if shader_module.compute_or_render.contains("render") || fuzzy(rng) {

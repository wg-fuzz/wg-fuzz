...
pub fn available_api_calls(resources: &ProgramResources, swarm: [bool; 89], fuzzy_prob: f64, terminate: bool) -> Vec<APICall> {
        ...
        for device in &adapter.devices {
            if !device.destroyed || fuzzy(&mut rng, fuzzy_prob) {
                ...
                for query_set in &device.query_sets {
                    ...
                    if !query_set.query_active || fuzzy(&mut rng, fuzzy_prob) {
                ...
                if device.render_bundle_encoders.len() < 3 || fuzzy(&mut rng, fuzzy_prob) {
                    available_api_calls.extend([CreateRenderBundleEncoder(device.clone())])
                }
                    
                if !device.error_scope_active || fuzzy(&mut rng, fuzzy_prob) {
                    available_api_calls.extend([PushRandomErrorScope(device.clone())])
                } else if device.error_scope_active || fuzzy(&mut rng, fuzzy_prob){
                ...
                add_manipulate_buffers(&mut available_api_calls, &mut rng, fuzzy_prob, resources, device);

                add_manipulate_textures(&mut available_api_calls, &mut rng, fuzzy_prob, resources, device);

                add_create_pipelines(&mut available_api_calls, &mut rng, fuzzy_prob, device);

                add_manipulate_bundles(&mut available_api_calls, &mut rng, fuzzy_prob, device);

                let all_command_encoders_submitted = add_manipulate_command_encoders(&mut available_api_calls, &mut rng, fuzzy_prob, device);

                if !device.error_scope_active && all_command_encoders_submitted || fuzzy(&mut rng, fuzzy_prob) {
